...
describe('UserController', () => {
    ...
    describe('Method: create', () => {
        ...
        test('Must throw an error if username already exists in the database', async () => {
            mockReq = {
                body: { username: 'luiz', password: '123' }
            };

            bcrypt.hash.mockResolvedValue('encrypte_pass');

            prisma.user.findUnique.mockResolvedValue({ id: 1, username: 'luiz' });

            await userController.create(mockReq, mockRes);

            expect(mockRes.status).toHaveBeenCalledWith(400);
            expect(mockRes.json).toHaveBeenCalledWith(
                expect.objectContaining({ message: 'This username is already registered. Try another one.' })
            );
        });
        ...
        test('Must return an error while trying to access the database', async () => {
    ...
    describe('Method: login', () => {
        ...
        test('Must throw errors when losing connection to the database', async () => {
            prisma.user.findUnique.mockRejectedValue(new Error());
            bcrypt.compare.mockResolvedValue(false);
            mockReq = { body: { username: 'xisculous', password: 'danone' } };
